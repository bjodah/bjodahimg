...
docker run --name bjodah-bjodahimgbase-build -e TERM -e HOST_UID=$(id -u) -e HOST_GID=$(id -g) -v $absolute_repo_path/_build:/build -w /build $REGISTRY_USER/$DOCKERFILE_NAME:$TAG /build/build-all-deb.sh | tee $(basename $0).log
