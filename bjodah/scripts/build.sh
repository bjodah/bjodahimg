...
cat <<'EOF' | docker run --rm -e RUNCMD="${3:-make}" -e TERM -e HOST_UID=$(id -u) -e HOST_GID=$(id -g) -v $ABS_INPUT_PATH:/input:ro -v $ABS_OUTPUT_PATH:/output -w /output -i ${5:-bjodah/bjodahimg}:${4:-latest} bash -x
cp -rau /input/. .
